export const conseilmunicipal = [
  {
    id: 1,
    info:
      "Le Conseil municipal est l'assemblée d'élus qui est chargée de gérer les affaires de la commune. Les Conseillers Municipaux au nombre de trente et un (31) sont les personnes qui siègent au Conseil municipal. Ils sont élus au suffrage universel direct pour un mandat de 5 ans renouvelable indéfiniment. Le Conseil Municipal est présidé par le Maire qui est élu par le Conseil au scrutin majoritaire uninominal. L'élection des Adjoints a lieu lors d'un scrutin de liste à la majorité absolue."
  }
];

export const attributions = [
  {
    id: 1,
    info: `Le Conseil municipal représente les habitants. Ses attributions sont très larges et se charge de régler par ses délibérations les affaires de la commune. Cette compétence s’étend à de nombreux domaines. Le Conseil municipal donne son avis toutes les fois qu’il est requis par les textes ou par le représentant de l’État.`
  },
  {
    id: 2,
    info: `Il émet des vœux sur tous les sujets d’intérêt local : il vote le budget, approuve le compte administratif (budget exécuté), il est compétent pour créer et supprimer des services publics municipaux, pour décider des travaux, pour gérer le patrimoine communal, pour accorder des aides favorisant le développement économique. Le conseil exerce ses compétences en adoptant des "délibérations". Ce terme désigne ici les mesures votées. Il peut former des commissions disposant d’un pouvoir d’étude des dossiers.`
  },
  {
    id: 3,
    info: `Le conseil municipal doit se réunir au moins une fois par trimestre et l’ordre du jour, fixé par la Municipalité doit être communiqué avant le début de la séance. Celle-ci est ouvert au public sauf si l’assemblée décide le huis clos ou si le Maire exerce son pouvoir de "police des séances", notamment en cas d’agitation, et restreint l’accès du public aux débats. En cas de dysfonctionnement grave, le Conseil municipal peut être dissous par décret en Conseil des ministres.`
  }
];
export const municipalite = [
  {
    id: 1,
    info:
      "La Municipalité de la commune d'Aboisso comprend cinq (5) membres. Elle est l’organe exécutif de la commune élue par le conseil municipal à sa première réunion, Elle est chargée de l’établissement de l’ordre du jour des séances du conseil, de surveiller l'entrée des impôts, des taxes et des droits municipaux, de coordonner les actions et les opérations de développement plus particulièrement de celles relatives à la participation des populations et prépare également toutes les opérations préliminaires à l’attribution de marché par le conseil municipal et de suivre l’exécution du programme triennal et de toutes les décisions du conseil."
  }
];

export const conseillers = [
  {
    id: 1,
    name: "N’GOUAN JEREMIE"
  },
  {
    id: 2,
    name: "BILE KHASSY ALPHONSE "
  },
  {
    id: 3,
    name: "BENIE KAKOU JEAN PAUL"
  },
  {
    id: 4,
    name: "KOFFI EKUE EMILIENNE"
  },
  {
    id: 5,
    name: "ADJAFFI ANGELINE"
  },
  {
    id: 6,
    name: "DOUMBIA DAOUDA"
  },
  {
    id: 7,
    name: "AGBIA N’GAMAN LUCIEN"
  },
  {
    id: 8,
    name: "KASSI DAMOH JULIEN SYLVAIN"
  },
  {
    id: 9,
    name: "SANHOU DJEDOU"
  },
  {
    id: 10,
    name: "N’GUESSAN KOUAME"
  },
  {
    id: 11,
    name: "AMANGOUA ATCHAUD ALBERT"
  },
  {
    id: 12,
    name: "ZOUMANA SANGARE"
  },
  {
    id: 13,
    name: "YANGBA DIORO EMMA JACQUELINE GOMON"
  },
  {
    id: 14,
    name: "KOFFI ANO N’GUESSAN"
  },
  {
    id: 15,
    name: "BORBA KOUTOUA"
  },
  {
    id: 16,
    name: "AKA EDJOLOHI ETIENNE"
  },
  {
    id: 16,
    name: "SORO KOLOTCHELEMA ROSE"
  },
  {
    id: 17,
    name: "COULOUD JEAN YVES HERVE"
  },
  {
    id: 18,
    name: "MALAN AGNIMAN ADJOBA"
  },
  {
    id: 19,
    name: "OUATTARA MAKAFIN DITE MARIAME"
  },
  {
    id: 20,
    name: "GUEYE AMADOU BOUBAKAR"
  },
  {
    id: 21,
    name: "BONSSOMBRA KASSI"
  },
  {
    id: 22,
    name: "CISSE ABOUBAKARI"
  },
  {
    id: 23,
    name: "BROU EDGAR VINCENT"
  },
  {
    id: 24,
    name: "DIAKITE FEDE"
  },
  {
    id: 25,
    name: "KONE ABIBA EPSE COULIBALY"
  },
  {
    id: 26,
    name: "MAMADOU KANO"
  },
  {
    id: 27,
    name: "ABDOULAYE TRAORE"
  },
  {
    id: 28,
    name: "AKA TCHEKOU"
  },
  {
    id: 29,
    name: "KOMAN OBBA CHRISTINE"
  },
  {
    id: 30,
    name: "EPKONON KOUAO JEAN CLAUDE"
  }
];
